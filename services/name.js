/**
 * let rn = new gCaseName('фам им от');
 * rn.fullName(rn.gcaseRod); // return
 */
...
const NameProcessor = {
  ...
  prepareRules: function() {
    for(let type in this.rules) {
      for(let key in this.rules[type]) {
  ...
  word: function(word, sex, wordType, gcase) {
    ...
    this.init();
    let rules = this.rules[wordType];

    if (rules.exceptions) {
      let pick = this.pick(word, sex, gcase, rules.exceptions, true);
      if (pick) return pick;
    }
    let pick = this.pick(word, sex, gcase, rules.suffixes, false);
    return pick || word;
  },
  ...
  ruleMatch: function(word, sex, rule, matchWholeWord) {
    ...
    for(let i = 0, n = rule.test.length; i < n; i++) {
      let test = matchWholeWord ? word : word.substr(Math.max(word.length - rule.test[i].length, 0));
...
// new gCaseName('Козлов Евгений Павлович')         // годится обычная форма
// new gCaseName('Евгений Павлович Козлов')        // в таком виде тоже
// new gCaseName('Козлов', 'Евгений')             // можно явно указать составляющие
// new gCaseName('Кунтидия', 'Убиреко', '', 'f') // можно явно указать пол ('m' или 'f')
let gCaseName = function(lastName, firstName, middleName, sex) {
...
gCaseName.prototype = {
  sexM:      NameProcessor.sexM,
  sexF:      NameProcessor.sexF,
  gcaseIm:   NameProcessor.gcaseIm,   gcaseNom: NameProcessor.gcaseNom, // именительный
  gcaseRod:  NameProcessor.gcaseRod,  gcaseGen: NameProcessor.gcaseGen, // родительный
  gcaseDat:  NameProcessor.gcaseDat,                                    // дательный
  gcaseVin:  NameProcessor.gcaseVin,  gcaseAcc: NameProcessor.gcaseAcc, // винительный
  gcaseTvor: NameProcessor.gcaseTvor, gcaseIns: NameProcessor.gcaseIns, // творительный
  gcasePred: NameProcessor.gcasePred, gcasePos: NameProcessor.gcasePos, // предложный
  ...
  lastName: function(gcase) {
    return NameProcessor.word(this.ln, this.sex, 'lastName', gcase);
  },
  firstName: function(gcase) {
    return NameProcessor.word(this.fn, this.sex, 'firstName', gcase);
  },
  middleName: function(gcase) {
    return NameProcessor.word(this.mn, this.sex, 'middleName', gcase);
  }
};
